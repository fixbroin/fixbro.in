...
import { doc, setDoc, Timestamp, getDoc, onSnapshot, collection, query, where, getDocs, limit, runTransaction, writeBatch, or } from "firebase/firestore";
 // Added onSnapshot, collection, query, where, getDocs, limit, runTransaction, writeBatch, or
...
export const AuthProvider: React.FC<PropsWithChildren> = ({ children }) => {
  ...
  const completeProfileSetup = useCallback(async (details: { fullName: string; email?: string; mobileNumber?: string }) => {
    ...
    try {
      ...
      // Trigger Push Notification for referrer if applicable
      if (referrerId) {
        triggerPushNotification({
          userId: referrerId,
          title: "New Referral Signup!",
          body: `${details.fullName} has signed up using your code. You'll get your bonus when they complete their first booking.`,
          href: '/referral'
        });
      }
