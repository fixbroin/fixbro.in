    ...
    <Card key={booking.id} className="mb-4 shadow-sm flex flex-col">
      ...
      <CardFooter className="p-4 pt-0 flex flex-col gap-3">
        ...
        <div className="flex gap-2">
          {booking.providerId ? (
            <>
              ...
              >
                <Users className="mr-1 h-3 w-3 shrink-0" /> Reassign
              </Button>
              <AlertDialog>
                <AlertDialogTrigger asChild>
                  <Button 
                    ...
                    disabled={isUpdatingStatus === booking.id || isLoadingAppSettings}
                  >
                    <XIcon className="mr-1 h-3 w-3 shrink-0" /> Unassign
                  </Button>
                </AlertDialogTrigger>
                <AlertDialogContent>
            ...
            >
              <UserCheck2 className="mr-1.5 h-3.5 w-3.5" /> Assign Provider
            </Button>
          ) : null}
        </div>
        ...
        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm" className="flex-1 text-xs h-9" onClick={() => handleViewDetails(booking)}>
            <Eye className="h-3.5 w-3.5 mr-1"/> Details
          </Button>
          <Button variant="outline" size="sm" className="flex-1 text-xs h-9" onClick={() => handleEditBooking(booking.id!)}>
            <Edit className="h-3.5 w-3.5 mr-1"/> Edit
          </Button>
          <AlertDialog>
            <AlertDialogTrigger asChild>
              <Button variant="destructive" size="icon" className="h-9 w-9 shrink-0" title="Delete Booking" disabled={isDeleting === booking.id || !booking.id}>
                          ...
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            {booking.providerId ? (
                              <Button 
                                ...
                                disabled={isUpdatingStatus === booking.id || isLoadingAppSettings}
                              >
                                <Users className="mr-1.5 h-3.5 w-3.5" /> Reassign Provider
                              </Button>
                            ) : (booking.status === "Confirmed" || booking.status === "Pending Payment" || booking.status === "Rescheduled") ? (
                              <Button 
                                variant="default" 
                              ...
                              <AlertDialog>
                                <AlertDialogTrigger asChild>
                                  <Button 
                                    ...
                                    disabled={isUpdatingStatus === booking.id || isLoadingAppSettings}
                                  >
                                    <XIcon className="mr-1.5 h-3.5 w-3.5" /> Unassign Provider
                                  </Button>
                                </AlertDialogTrigger>
                                <AlertDialogContent>
                                  <AlertDialogHeader>
                                  ...
                                  <AlertDialogFooter>
                                    <AlertDialogCancel>Cancel</AlertDialogCancel>
                                    <AlertDialogAction 
                                      onClick={() => handleUnassignProvider(booking.id!)} 
