...
export function HeroCarousel() {
  ...
  React.useEffect(() => {
    const fetchSlides = async () => {
      ...
      const cachedSlides = getCache<FirestoreSlide[]>(cacheKey);
      ...
      try {
        ...
        setCache(cacheKey, fetchedSlides); // Save to cache
      } catch (error) {
      ...
      >
        ...
        {slides.length > 1 && (
          <>
            ...
            <CarouselNext className="absolute -right-4 top-1/2 -translate-y-1/2 h-6 w-6 sm:h-8 sm:w-8" />
          </>
        )}
      </Carousel>
    </div>
